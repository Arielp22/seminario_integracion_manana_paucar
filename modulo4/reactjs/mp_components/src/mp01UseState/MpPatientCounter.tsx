// src/mp01UseState/MpPatientCounter.tsx

import { useState } from 'react'

interface MpPatientCounterProps {
  initialValue?: number
  step?: number
  label?: string
}

export default function MpPatientCounter({
  initialValue = 0,
  step = 1,
  label = 'Pacientes',
}: MpPatientCounterProps) {
  const [count, setCount] = useState(initialValue)

  function increment() { setCount(count + step) }
  function decrement() { setCount(count - step) }
