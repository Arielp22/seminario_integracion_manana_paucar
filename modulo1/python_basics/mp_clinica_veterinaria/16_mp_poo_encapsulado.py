...
class Mascota:
    def __init__(self, titular, saldo_inicial=0):
        ...
        self.__historial = []
        self.__activa   = True
        self.__registrar(f"Cuenta creada con {saldo_inicial}ml")
    ...
    def depositar(self, cantidad):
        ...
        self.__registrar(f"Deposito: +{cantidad}ml")
        return self
    ...
    def retirar(self, cantidad):
        ...
        if cantidad > self.__saldo:
            raise ValueError(f"Saldo insuficiente (disponible: {self.__saldo}ml)")
        self.__saldo -= cantidad
        self.__registrar(f"Retiro: -{cantidad}ml")
        return self

    def transferir(self, destino, cantidad):
        self.retirar(cantidad)
        destino.depositar(cantidad)
        self.__registrar(f"Transferencia a {destino.titular}: -{cantidad}ml")
        return self
    ...
    def __str__(self):
        return f"Cuenta({self.titular}: {self.__saldo}ml)"

# Uso
c1 = Mascota("Luna", 1000)
c2 = Mascota("Roco", 500)

c1.depositar(500).retirar(200)     # encadenamiento — depositar y retirar devuelven self
c1.transferir(c2, 300)

print(c1)    # Cuenta(Luna: 1000ml)
print(c2)    # Cuenta(Roco: 800ml)
print(f"Saldo Ana: {c1.saldo}ml")   # acceso como atributo (property)
